...
namespace ee669 {
    ...
    public:
        ...
        template<typename Iterator, typename T = typename std::iterator_traits<Iterator>::value_type>
        static std::pair<double, double> calcPSNR(Iterator first, Iterator last,
            Iterator oriFirst, Iterator oriLast, T max = std::numeric_limits<T>::max());
    };

    template<typename Iterator, typename T>
    std::unordered_map<T, uint64_t>
    Utils::stat(Iterator first, Iterator last) {
    ...
    template<typename T>
    double
    Utils::calcEntropy(const std::unordered_map<T, uint64_t> &stat) {
    ...
    Utils::calcPSNR(Iterator first, Iterator last, Iterator oriFirst, Iterator oriLast, T max) {
        static_assert(std::is_integral<T>::value, "Template type T must be integral");
        ...
        double mse = static_cast<double>(sse) / size;
        return std::make_pair(10 * std::log10(static_cast<double>(max) * max / mse), mse);
    }
}
