...
namespace vitter {
	...
	void write_to_file(std::ostream *file, std::queue<char> *code_write) {
		unsigned char temp;
		temp = temp & 0x00;
		for (int i=0; i<8; i++) {
	...
	void encode(node **tree, unsigned char symbol, unsigned char *dictionary, std::queue<char> *code_write, std::ostream *file, node **nyt) {
		// symbol exist
		if (dictionary[(int) symbol] == 0x01) {
			char do_code[1];
			do_code[0] = '\0';
	...
	bool read_from_file(std::istream *file, std::queue<char> *code_read) {
		char temp;
	...
	void write_to_file_instansly(std::ostream *file, unsigned char symbol) {
		*file << symbol;

		return;
	}

	void decode(node **tree, unsigned char *dictionary, std::queue<char> *code_read, std::istream *file, std::ostream *out_file, node **nyt, bool *oke, short offset) {
		// 4 byte
	...
	bool read_from_file_instansly(std::istream *file, unsigned char *symbol) {
		char temp;
		if ((*file).get(temp)) {
			symbol[0] = temp;
