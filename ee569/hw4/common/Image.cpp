...
Image *Image::rasterMap(const std::function<PIXEL_TYPE *(const unsigned int, const unsigned int)>& fn) const {
    ...
    this->rasterScan([pppuhMappedImage, fn](const unsigned int i, const unsigned int j) {
        auto result = fn(i, j);
        if (result != nullptr) {
...
void Image::concurrentScan(const std::function<void(const unsigned int, const unsigned int)>& fn,
    ...
    auto pcFuture = new std::vector<std::future<void>>;
    const auto task = [=](unsigned int i) {
    ...
    for (auto i = 0u; i < threads; ++i) {
        pcFuture->push_back(std::async(task, i));
    }
    for (auto i = 0u; i < threads; ++i) {
        pcFuture->at(i).get();
    }
...
Image *Image::concurrentMap(const std::function<PIXEL_TYPE *(const unsigned int, const unsigned int)>& fn,
                            const uint8_t threads) const {
    if (this->getImage() == nullptr) {
        return nullptr;
    }
